# scDissector scripts
source("D:/Google Drive/scDissector-master/R/load_dataset_and_model.R")
source("D:/Google Drive/scDissector-master/R/projector.r")
source("D:/Google Drive/scDissector-master/R/insilico_sorter.r")
source("D:/Google Drive/sc_scripts/umitab_utils.r")
# further references included in scripts to "D:/Google Drive/scDissector-master/inst/extdata/"



# check if scDissector sources needed
# check ldm
# compiled samples
"D:/Google Drive/scRNAseq_analysis/compiled/clustering_data_lung4/data/stroma_lung_compiled"

# load ldm
# change cluster set
# resave ldm

# remove ldm from folder

# copy ldm loading script that downloads data from jeromes paper
...
lig_heatmap_genes <- "D:/Google Drive/Paper website/Scripts/inputs/immunomod/lig_rec_ligands.txt"
ecm_heatmap_genes <- "D:/Google Drive/Paper website/Scripts/inputs/immunomod/ecm.txt"
...
stroma_ldm <- "D:/Google Drive/Paper website/Scripts/inputs/ldm_consented_2000_v2.RData"
# LCAM source codes
...
make_truth_plots(ldm,clusters_to_exclude=clusters_to_exclude,clust_num=250,samp_num=100,
                 gene_list2,gene_list1,main_title="modules_s5a",zlim=c(0,4))
...
########################
########################
# ldm is adjusted after this point and must be reloaded if running code from before this point



# create a vector that maps from cells to new cluster
########################
########################
########################
########################
...
ldm$dataset$cell_to_cluster <- gsub("^26$","22",ldm$dataset$cell_to_cluster)
ldm$dataset$cell_to_cluster <- gsub("^24$","22",ldm$dataset$cell_to_cluster)
...
for (num in i){
  cluster1 <- colnames(u)[colnames(u)%in%names(cluster_id[cluster_id == num])]
...
cell_metadata <- cbind(cell_metadata,sample_annots[as.character(cell_metadata$sample_ID),
